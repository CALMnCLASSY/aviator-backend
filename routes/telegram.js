...
// Track visitor navigation and send to admin
router.post('/track-navigation', async (req, res) => {
    try {
        ...
        console.log('📊 Navigation tracking received:', { sessionId, email, phone });
        
        // Format visitor summary for Telegram
        const trackingMessage = `🔍 VISITOR NAVIGATION TRACKING
...
📍 SECTIONS VISITED:
${visitData.sectionsVisited.map(section => `• ${section}`).join('\n') || 'No sections tracked yet'}

📊 ACTIVITY SUMMARY:
• Total Interactions: ${visitData.totalInteractions}
• Session Duration: ${Math.round(visitData.sessionDuration / 1000)}s
• Last Activity: ${new Date(visitData.lastActivity).toLocaleTimeString()}

${latestInteraction ? `🔥 LATEST ACTION:
• Type: ${latestInteraction.type}
• Target: ${latestInteraction.trackId}
• Time: ${new Date(latestInteraction.timestamp).toLocaleTimeString()}` : ''}

🌐 Browser: ${userAgent.split(' ')[0]}
⏰ Time: ${new Date(timestamp).toLocaleString()}`;

        // Send to admin
        await sendTelegramMessage(telegramChatId, trackingMessage);
        
        res.json({ success: true, message: 'Navigation data tracked' });
...
// Capture phone number when M-Pesa button is clicked
router.post('/phone-capture', async (req, res) => {
    try {
        const {
            sessionId,
            email,
        ...
        console.log('📱 Phone capture triggered:', { phone, context, email });
        
        // Format phone capture alert for Telegram
        const phoneMessage = `🚨 PHONE NUMBER CAPTURED!
...
📊 VISITOR ACTIVITY:
• Sections Visited: ${sectionsVisited.length}
• Total Interactions: ${totalInteractions}
• Session Duration: ${Math.round(sessionDuration / 1000)}s

📍 Journey:
${sectionsVisited.map((section, index) => `${index + 1}. ${section}`).join('\n') || 'No sections tracked'}

⚡ ACTION REQUIRED: Follow up with this visitor!
💡 They clicked M-Pesa but may not complete payment

⏰ Time: ${new Date(timestamp).toLocaleString()}`;

        // Send urgent notification to admin
        await sendTelegramMessage(telegramChatId, phoneMessage);
        
        // Log for follow-up tracking
        console.log(`📞 FOLLOW-UP REQUIRED: ${phone} (${email}) - ${context}`);
        
        res.json({ success: true, message: 'Phone capture sent to admin' });
